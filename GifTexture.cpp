...
GifTexture::GifTexture()
	:pSampler_(nullptr),pSRV_(nullptr),currSamplerState(0),currSRV(0),imageNum(0)
{
}
...
HRESULT GifTexture::Load(string filename)
{
	...
	ScratchImage image;
	HRESULT hr;
	hr = LoadFromWICFile(wtext, WIC_FLAGS::WIC_FLAGS_ALL_FRAMES, &metadata, image);
	imgSize_ = XMFLOAT2{ (float)image.GetImages()->width, (float)image.GetImages()->height };
	/////////

	imageNum = image.GetImageCount();
